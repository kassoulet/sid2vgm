...
impl Converter {
    ...
    pub fn convert(
        ...
        // The emulator may run slightly past the requested cycle count; drop
        // events outside the capture window so no wait extends beyond the
        // header's total sample count.
        let mut vgm_events: Vec<(u32, VgmCommand)> = all_events
            .into_iter()
            .filter(|e| e.cycle < total_cycles)
            .map(|e| {
        ...
        let event_count = vgm_events.len();
