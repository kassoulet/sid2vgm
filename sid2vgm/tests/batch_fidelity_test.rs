...
#[test]
fn test_batch_fidelity() {
    let sids_dir = "tests/sids";
    let vgms_dir = "tests/vgms";
    let vgm_wavs_dir = "tests/vgm-wavs";
    let wavs_dir = "tests/wavs";

    fs::create_dir_all(vgms_dir).unwrap();
    fs::create_dir_all(vgm_wavs_dir).unwrap();
    fs::create_dir_all(wavs_dir).unwrap();

    let sl = songlengths::SongLengths::load(&format!("{}/Songlengths.txt", sids_dir));

    let entries = fs::read_dir(sids_dir).expect("Failed to read sids dir");

    for entry in entries {
        let entry = entry.unwrap();
        let path = entry.path();

        if path.is_file() && path.extension().and_then(|s| s.to_str()) == Some("sid") {
            let file_name = path.file_stem().unwrap().to_str().unwrap();
            let vgm_path = format!("{}/{}.vgm", vgms_dir, file_name);
            ...
            assert!(
                status.success(),
                "sidplayfp failed to render reference: {:?}",
                path
            );
            ...
            assert!(status.success(), "sid2vgm failed: {:?}", path);
...
fn compare_wavs(ref_path: &str, test_path: &str) {
    ...
    println!("RMS Error for {}: {}", ref_path, rms_error);
